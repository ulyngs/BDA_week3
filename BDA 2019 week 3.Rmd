...
# BDA Week 3 notes
In the first half of the lecture we look at how to plot two variables together, using some extensions to a scatterplot. We then look at how to plot error bars, using the 'error_bars()' command from the 'rBDA' package. In the second half we look at how to model non-linear relationships in bivariate data, using LOESS curves and logarithms.

This week is more technically demanding than the last two - if you have any issues, please shoot me an email at ulrik.lyngs@cs.ox.ac.uk.
...
mydata2 %>% 
  gather() %>% #this stack the x and y columns on top of each other and adds a 'key' column that keep track of where it came from
  ggplot(aes(value)) +
    geom_histogram() +
    facet_wrap(~key,
               scales = 'free')
...
df_summary <- my_results$df.summary # the summary information for the bins, with all of the calculated variables
df_extended <- my_results$df # the original dataframe (mydata2) we passed to the function, with the bin values attached
...
ggplot() +
  geom_point(data = df_extended, # put the original data on the plot - gives an idea also of how many values are in each bin
             aes(x, y),
             color = 'blue',
             alpha = 0.02,
             size = 1) +
  geom_point(data = df_summary, # each point shows a bin - with the mean x value and the mean y value
             aes(xmean, ymean),
             size = 2) + 
